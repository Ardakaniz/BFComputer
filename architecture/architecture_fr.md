...
Le Brainf*ck est donc composé de huit instructions élémentaires permettant de manipuler une mémoire, effectués des boucles et interagir avec des entrées/sorties.
...
Cependant, ce qui complexifie la tâche est la gestion des boucles. Dans un langage d’assembleur normal, il n’existe pas de boucle à proprement parlé mais des ‘sauts’ avec ainsi l’adresse d’arrivée indiquée dans l’instruction. Ici le challenge a été de trouver un mécanisme permettant à l’ordinateur d’aller chercher le début ou la fin de la boucle correspondante -- non connue d'avance sauf éventuellement en rajoutant des données aux instructions mais le but était vraiment que l'ordinateur se débrouille avec un code BF brut.    
De plus, chaque instruction de boucle va être stocké dans une mémoire cache parallèle à celle utilisable pour l’utilisateur.
...
/!\ La lecture est une opération asynchrone : dès que le signal de controle est actif, la valeur est mise sur le bus ; tandis que l'écriture se fait de manière synchrone : la valeur ne sera dans la RAM que au prochain front montant de l'horloge
...
#### Les entrées/sorties
Plusieurs périphériques peuvent faire office d’entrée/sorties, une seule et même interface est exposée, controlé par ces signaux:
- Pour la sortie :
  - **OUT** : transmet la valeur de la cellule actuelle
  - **S0**  : sélectionne soit la valeur de l'ALU (si la valeur a été changée), soit la valeur du bus (si la valeur a été inchangée)
- Pour l’entrée :
...
Nous avons donc pour cela une ROM principale avec 1k cellules de 25 bits. 1k cellules correspondent à 10 bits correspondant à quatre éléments résumé dans le tableau ci dessous :
...
##### Program ROM
La ROM qui contient le programme à exécuter. Son contenu peut être changé via le programmateur de ROM dont une partie de ce projet est aussi dédiée. Les instructions défilent suivant la valeur du PC.
...
##### Zero Checker 
Permet d’indiquer à l’ordinateur quand le LPC est à zéro. (Réaliser à partir de portes ET and d'un inverseur à la toute fin)
...
La gestion des boucles est assez complexe : en effet, l'ordinateur réserve une partie de la mémoire vive comme cache pour enregistrer où sont placé les instructions de boucle.    
...
> /!\ Cette gestion possède une limitation : le code BF ne peut pas commencer par une boucle dès la première instruction, pour cela il faut utiliser une instruction fantôme en premier lieu comme `><`